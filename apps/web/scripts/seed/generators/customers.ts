 ...
 */
function generateNeighboringCountryAddress(): {
  address: string;
  city: string;
  postalCode: string;
  country: string;
...
export function generateCustomers(
  ...
  const activeSessionCount = Math.floor(individualCustomers.length * 0.15);
  const customersWithSessions = pickRandomMultiple(individualCustomers, activeSessionCount);

  for (const customer of customersWithSessions) {
