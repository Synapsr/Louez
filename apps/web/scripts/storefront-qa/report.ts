...
<p class="meta">Copie initiale du ${escape(report.snapshotAt)} · commit ${escape(report.revision.slice(0, 10))} + ${report.dirtyFiles} fichiers modifiés. Données créées le ${escape(report.createdAt)}. Pendant <code>dev</code>, les modifications du checkout sont synchronisées automatiquement.</p>
<nav><a href="${report.mailUrl}" target="_blank" rel="noopener">Boîte email de test</a><a href="${report.fixtureUrl}" target="_blank" rel="noopener">Simulateurs et erreurs</a><a href="https://app.louez-qa.localify" target="_blank" rel="noopener">Dashboard</a>${report.verificationUrl ? `<a href="${escape(report.verificationUrl)}">Bilan initial et anomalies</a>` : ""}</nav>
...
<div class="controls"><label>Rechercher <input id="search" placeholder="Assurance, caution…"></label><label>Boutique <select id="store"><option value="">Toutes</option>${report.stores.map((s) => `<option value="${s.slug}">${escape(s.name)}</option>`).join("")}</select></label><label>Résultat <select id="status"><option value="">Tous</option><option>À tester</option><option>Validé</option><option>Anomalie</option><option>Bloqué</option></select></label><button id="export">Exporter mes résultats JSON</button></div><p id="counts" role="status"></p><p id="empty" hidden>Aucun scénario ne correspond aux filtres sélectionnés.</p>
...
const cards=[...document.querySelectorAll('article')];const search=document.querySelector('#search'),store=document.querySelector('#store'),status=document.querySelector('#status');
function counts(){const visible=cards.filter(c=>!c.hidden).length;document.querySelector('#counts').textContent=visible+' / '+cards.length+' scénarios affichés · '+Object.values(saved).filter(s=>s.result==='Validé').length+' validés manuellement au total';document.querySelector('#empty').hidden=visible!==0}
for(const card of cards){const id=card.dataset.id,select=card.querySelector('select'),notes=card.querySelector('textarea');if(saved[id]){select.value=saved[id].result;notes.value=saved[id].notes}function save(){saved[id]={result:select.value,notes:notes.value,at:new Date().toISOString(),viewport:[innerWidth,innerHeight],browser:navigator.userAgent};localStorage.setItem(key,JSON.stringify(saved));filter()}select.onchange=save;notes.onchange=save}
function filter(){for(const card of cards)card.hidden=Boolean((store.value&&card.dataset.store!==store.value)||(status.value&&card.querySelector('.result').value!==status.value)||!card.textContent.toLowerCase().includes(search.value.toLowerCase()));counts()}search.oninput=filter;store.onchange=filter;status.onchange=filter;filter();
