...
import { and, asc, eq, exists, gte, inArray, like, ne, notExists, or, sql } from "drizzle-orm";
import { z } from "zod";
...
import type { BookingAttributeAxis, ProductAvailability } from "@louez/types";
...
const buildProductConditions = ({
  ...
  "storeId" | "category" | "search" | "quantity" | "attributes" | "excludeProductId"
>) => {
  ...
  const attributeConditions = Object.entries(attributes ?? {}).flatMap(([axis, values]) => {
    if (values.length === 0) return [];
    const unitValue = sql`json_unquote(json_extract(${productUnits.attributes}, ${attributeJsonPath(axis)}))`;
    return [inArray(unitValue, [...values])];
  });
  if (attributeConditions.length > 0) {
    const matchingUnits = and(
...
const getFilteredAvailableQuantity = (
  product: Pick<ProductAvailability, "availableQuantity" | "combinations">,
  attributes: CatalogAttributeFilters | undefined,
): number | null => {
  const filters = Object.entries(attributes ?? {}).filter(([, values]) => values.length > 0);
  if (filters.length === 0) return product.availableQuantity;
  const matchingQuantity = (product.combinations ?? [])
    .filter((combination) =>
      filters.every(([axis, values]) =>
        values.includes(combination.selectedAttributes[axis] ?? ""),
      ),
    )
    .reduce((quantity, combination) => quantity + combination.availableQuantity, 0);
  return product.availableQuantity === null
    ? matchingQuantity
    : Math.min(product.availableQuantity, matchingQuantity);
};
  ...
  if (startDate && endDate && candidates.length > 0 && (availableOnly || (quantity ?? 1) > 1)) {
    ...
    availabilityById = new Map(
      availability.products.map((product) => [
        product.productId,
        getFilteredAvailableQuantity(product, attributes),
      ]),
    );
  }
