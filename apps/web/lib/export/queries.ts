...
function reservationsToRows(data: Awaited<ReturnType<typeof queryReservationsData>>): string[][] {
  return data.map((row) => [
    str(row.number),
    str(row.status),
...
export async function queryExportData(storeId: string, params: ExportParams): Promise<ExportData> {
  switch (params.type) {
    case "payments": {
