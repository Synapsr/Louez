...
/**
 * Whether the phone receptionist is usable at the PLATFORM level: the operator
 ...
 */
export function isPhoneReceptionistConfigured(): boolean {
  return isVoiceConfigured() && isAIChatConfigured()
}

/**
 * Single source of truth for whether the receptionist is active for a store:
 ...
 */
export async function isPhoneReceptionistActiveForStore(store: {
  id: string
  aiPhoneSettings: AiPhoneSettings | null
}): Promise<boolean> {
  if (!store.aiPhoneSettings?.enabled || !isPhoneReceptionistConfigured()) {
    return false
  }
