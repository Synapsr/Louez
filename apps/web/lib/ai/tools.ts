      ...
      execute: async ({ status, categoryId, search }) => {
        ...
        return {
          products: rows.map((product) => ({
            ...product,
            quantity: product.stockKind === "untracked" ? null : product.quantity,
          })),
      ...
      execute: async ({ productId }) => {
        ...
        const effectiveQuantity =
          product.stockKind === "untracked"
            ? null
            : product.trackUnits
              ? product.units.filter((unit) => unit.lifecycleStatus === "active").length
              : product.quantity;
      ...
      execute: async ({ reservationId, status: newStatus }) => {
        ...
        try {
          const transitionResult = await db.transaction(async (tx) => {
            ...
            const lockedAllowed = getAllowedReservationStatusTransitions(lockedReservation.status);
            if (!canTransitionReservationStatus(lockedReservation.status, newStatus)) {
              return {
                ok: false as const,
            ...
            await tx
              ...
              .set(updateData)
              .where(
                and(eq(reservations.id, reservationId), eq(reservations.storeId, ctx.storeId)),
              );
    ...
    delete_payment: tool({
      ...
      execute: async ({ paymentId }) => {
        ...
        if (payment.method === "stripe") {
          return {
            error: "Cannot delete a Stripe payment. Use the Stripe dashboard for refunds.",
          };
        }
    ...
    calendar_upcoming: tool({
      ...
      inputSchema: z.object({
        days: z
          .number()
          .optional()
          .describe("Number of days to cover, counted from today (default 7). 1 = today only."),
      }),
      execute: async ({ days }) => {
        requirePermission(ctx, "reservations", "read");
