...
const STATUS_COLORS: Record<ReservationStatus, string> = {
  pending:
    'bg-reservation-pending-soft border-reservation-pending-border border-l-reservation-pending text-reservation-pending',
  confirmed:
    'bg-reservation-confirmed-soft border-reservation-confirmed-border border-l-reservation-confirmed text-reservation-confirmed',
  ongoing:
    'bg-reservation-ongoing-soft border-reservation-ongoing-border border-l-reservation-ongoing text-reservation-ongoing',
  completed:
    'bg-reservation-completed-soft border-reservation-completed-border border-l-reservation-completed text-reservation-completed',
  cancelled:
    'bg-reservation-cancelled-soft border-reservation-cancelled-border border-l-reservation-cancelled text-reservation-cancelled',
  rejected:
    'bg-reservation-rejected-soft border-reservation-rejected-border border-l-reservation-rejected text-reservation-rejected',
  quote:
    'bg-reservation-quote-soft border-reservation-quote-border border-l-reservation-quote text-reservation-quote',
  declined:
    'bg-reservation-declined-soft border-reservation-declined-border border-l-reservation-declined text-reservation-declined',
};
...
function getReservationWeekSpan(
  ...
  const startCol = Math.floor(
    (effectiveStart.getTime() - weekStartDate.getTime()) /
      (24 * 60 * 60 * 1000),
  );
  const endCol = Math.floor(
    (effectiveEnd.getTime() - weekStartDate.getTime()) / (24 * 60 * 60 * 1000),
  );
...
function assignTracksToWeek(
  ...
  const tracks: boolean[][] = Array.from({ length: MAX_TRACKS }, () =>
    Array(7).fill(false),
  );
...
function SpanningReservationBar({
  ...
  const statusColor =
    STATUS_COLORS[reservation.status as ReservationStatus] ||
  ...
  return (
    <Tooltip>
      ...
      >
        ...
        {continuesAfter && <span className="ml-auto opacity-70">▸</span>}
      </TooltipTrigger>
                    ...
                    >
                      ...
                      {overflow > 0 && (
                        <Link
                          ...
                          className={cn(
                            'text-muted-foreground hover:text-foreground absolute right-1 bottom-1 left-1 text-center text-xs',
                      ...
                      {today && (
                        <div className="absolute right-1 bottom-1 left-1">
                          <div className="bg-primary/50 h-0.5 rounded-full" />
                        </div>
                      )}
                    </div>
                  );
