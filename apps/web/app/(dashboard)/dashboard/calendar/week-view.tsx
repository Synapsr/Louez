...
export function WeekView({ reservations, currentDate, selectedProductId }: WeekViewProps) {
    ...
    <div className="overflow-hidden rounded-lg border bg-card">
      <div className="overflow-x-auto">
        <div className="min-w-[700px]">
          ...
          {weekLayout.maxRows > 0 && (
            <div className="relative border-b bg-muted/10" style={{ height: spanningAreaHeight }}>
              ...
              <div className="pointer-events-none absolute inset-0 grid grid-cols-7">
                {weekDays.map((day, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      className={cn("border-r last:border-r-0", isWeekend && "bg-muted/10")}
                    />
                  );
                })}
              </div>
          ...
          <div className="grid grid-cols-7" style={{ minHeight: MIN_DAY_HEIGHT }}>
            {weekDays.map((day, dayIndex) => {
              ...
              const singleDayReservations = filteredReservations.filter((r) => {
                ...
                return (
                  isSameDay(start, day) && isSameDay(end, day) && reservationIncludesDay(r, day)
                );
              });
                ...
                >
                  ...
                  <div className="space-y-1">
                    {singleDayReservations.slice(0, 5).map((reservation) => (
                      <ReservationBar key={reservation.id} reservation={reservation} compact />
                    ))}
...
function SpanningBar({ spanning }: SpanningBarProps) {
  ...
  const _marginPercent = 0.5; // 0.5% margin on each side
