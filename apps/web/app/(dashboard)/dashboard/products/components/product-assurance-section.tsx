...
export function ProductAssuranceSection({ productId }: ProductAssuranceSectionProps) {
  ...
  if (loadErrorKey || !productStateQuery.data || 'error' in productStateQuery.data) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>{t('title')}</CardTitle>
  ...
  const hasValidMapping =
    !!state.product.tulipProductId && tulipProductById.has(state.product.tulipProductId)
          ...
          {!state.connected ? (
            <form className="space-y-4" onSubmit={handleConnect}>
              ...
              <div className="flex flex-wrap gap-2">
                <Button
                  type="submit"
                  disabled={connectMutation.isPending || apiKey.trim().length === 0}
                >
                ...
                <Button
                  ...
                  variant="outline"
                  render={
                    <a href={state.calendlyUrl} target="_blank" rel="noreferrer" />
                  }
                >
