...
export const CheckoutCompanySearchField = ({
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor="checkout-company-search">{t("companySearch.label")}</Label>
      <div className="relative">
        <SearchIcon className="text-muted-foreground pointer-events-none absolute top-1/2 start-3 z-10 size-4 -translate-y-1/2" />
        <Input
          id="checkout-company-search"
          type="search"
          autoComplete="off"
          className="[&_input]:ps-9 [&_input]:pe-9"
          placeholder={t("companySearch.placeholder")}
          value={query}
          onChange={(event) => handleQueryChange(event.target.value)}
        />
        {isSearching && (
          <Spinner className="text-muted-foreground pointer-events-none absolute top-1/2 end-3 z-10 size-4 -translate-y-1/2" />
        )}
      </div>
      <p className="text-muted-foreground text-sm">{t("companySearch.description")}</p>
            ...
            <li key={company.siren}>
              ...
              >
                <span className="flex flex-wrap items-center gap-2">
                  ...
                  {!company.isActive && (
                    <Badge variant="warning">{t("companySearch.closed")}</Badge>
                  )}
                </span>
      ...
      {hasSearched && !isSearching && results.length === 0 && (
        <p className="text-muted-foreground text-sm">{t("companySearch.noResults")}</p>
      )}
    </div>
  );
};
