...
export const CheckoutAdvisorVerificationPanel = ({
  ...
  useEffect(() => {
    ...
    startVerification();
  }, [showsSurface, hasError, gate.isStatusLoading, isHydrating, messages, startVerification]);
  ...
  useEffect(() => {
    ...
    const pulseOnId = window.setTimeout(() => setHighlightInput(true), reduce ? 400 : 950);
    const pulseOffId = window.setTimeout(() => setHighlightInput(false), reduce ? 1200 : 2450);
    return () => {
  ...
  return (
    <div ref={panelRef}>
      ...
      {gate.isValidated ? (
        <Card className="mb-4 border-success/40">
          <CardContent className="flex items-center gap-3 py-4">
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-foreground">{t("validatedTitle")}</p>
              <p className="text-xs text-muted-foreground">{t("validatedDescription")}</p>
            </div>
            <Button
              type="button"
              variant="outline"
        ...
        <Card className={cn("mb-4", isStale ? "border-warning/40" : "border-primary/30")}>
          <CardContent className="space-y-4 pt-6">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium text-foreground">{t("requiredTitle")}</p>
                <p className="text-xs text-muted-foreground">{t("requiredDescription")}</p>
              </div>
