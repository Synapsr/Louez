      ...
      <DialogPopup className="sm:max-w-md">
        ...
        <DialogPanel>
          ...
          >
            ...
            <span className="text-sm font-medium">{showQR ? t("hideQR") : t("showQR")}</span>
            {showQR ? (
          ...
          {showQR && (
            <div className="animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="flex flex-col items-center gap-4 rounded-xl border bg-white p-6 dark:bg-muted/30">
                {/* QR Code */}
                <div ref={qrRef} className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-black/5">
                  <QRCodeSVG
                    value={storeUrl}
                    size={160}
                    level="H"
                ...
                {/* Description */}
                <p className="text-center text-xs text-muted-foreground">{t("qrDescription")}</p>

                {/* Download Button */}
                <button onClick={handleDownloadQR} className="action-btn action-btn--primary gap-2">
