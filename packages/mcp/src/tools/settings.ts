  ...
  server.tool(
    ...
    'Update legal documents (terms of service, legal notice)',
    {
      cgv: z
        .string()
        .optional()
        .describe('General terms and conditions (HTML or text)'),
      legalNotice: z.string().optional().describe('Legal notice text'),
      includeCgvInContract: z
        .boolean()
        .optional()
        .describe('Include terms in rental contracts'),
    },
    async (updates) => {
      requirePermission(ctx, 'settings', 'write');
      ...
      if (updates.legalNotice !== undefined)
        updateData.legalNotice = updates.legalNotice;
