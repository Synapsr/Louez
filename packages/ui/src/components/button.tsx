...
const buttonVariants = cva(
  ...
  ],
  {
    variants: {
      ...
      size: {
        default:
          'h-9 gap-1.5 px-2.5 data-[icon-end=true]:pe-2 data-[icon-start=true]:ps-2',
...
function isIconElement(child: ReactNode): child is IconElement {
  return (
    ...
    (child.props as { 'data-slot'?: string })['data-slot'] === 'icon'
  );
}
...
function Button({
  ...
  const startSlot =
    isPending || startIcon ? (
      ...
      >
        {isPending ? (
          <Spinner data-slot="icon" className={iconClassName} />
        ) : (
          startIcon
        )}
      </AnimatedIconSwap>
  ...
  const content = hidesLabel
    ...
    : isPending && pendingContent != null
      ? pendingContent
      : idleLabel;
  const hasLabel = Array.isArray(content)
    ? content.length > 0
    : content != null;

  const layout = shouldReduceMotion ? false : ('position' as const);
  const hidden = (x: number) =>
    shouldReduceMotion ? { opacity: 0 } : { opacity: 0, x };
