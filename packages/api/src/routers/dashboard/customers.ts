...
const list = dashboardProcedure.input(listInputSchema).handler(async ({ context, input }) => {
  ...
  let query = db
    ...
    .leftJoin(
      reservations,
      and(eq(reservations.customerId, customers.id), eq(reservations.storeId, storeId)),
    )
    .where(whereClause)
