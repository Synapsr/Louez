...
import type { BaseContext, DashboardContext, StorefrontContext, StoreData } from "./context";
import { createAvailabilityMemo } from "./services/availability";
 ...
 */
export const dashboardProcedure = publicProcedure.use(async ({ context, next }) => {
 ...
 *
 * The store row is read once here, with the columns storefront services
 * need, and handed down through the context; a store still in onboarding is
 * invisible, like on every other storefront entry point.
 */
export const storefrontProcedure = publicProcedure.use(async ({ context, next }) => {
  ...
  const store = await db.query.stores.findFirst({
    ...
    where: and(eq(stores.slug, storeSlug), eq(stores.onboardingCompleted, true)),
  });
  ...
  return next({
    context: {
      ...
      store,
      customer,
      availabilityMemo: createAvailabilityMemo(),
 ...
 */
export const storefrontAuthProcedure = storefrontProcedure.use(async ({ context, next }) => {
  if (!context.customer) {
    throw new ORPCError("UNAUTHORIZED", {
