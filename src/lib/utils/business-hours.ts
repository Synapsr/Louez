 ...
 */
export function getDaySchedule(date: Date, businessHours: BusinessHours): DaySchedule {
...
export function isWithinBusinessHours(
  ...
  const daySchedule = getDaySchedule(date, businessHours)
...
export function getAvailableTimeSlots(
  ...
  const dateCheck = isDateAvailable(date, businessHours)
...
export function getNextAvailableDate(
  ...
  for (let i = 0; i < maxDaysToSearch; i++) {
    const check = isDateAvailable(currentDate, businessHours)
    if (check.available) {
      return currentDate
    }
