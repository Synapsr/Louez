...
- `reservation_item_units` gains real foreign keys (`reservation_item_id` cascade, `product_unit_id` restrict) so orphaned assignments become impossible instead of silently invisible.
