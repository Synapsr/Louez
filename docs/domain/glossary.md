...
**Louez commission**:
A per-reservation fee charged by Louez to a Store, metered through the pay-as-you-go ledger (`platform_fee`) and billed monthly. It is charged on reservations, never itemized on a Store's Invoices to its Customers.
...
**Invoice (Facture)**:
An immutable legal billing document issued by a Store (as legal seller) to a Customer for a paid reservation, generated by Louez on the Store's behalf and transmitted electronically through the Store's PDP.
_Avoid_: accounting export, receipt, bill, contract

**Credit note (Avoir)**:
An invoice-class document that reverses all or part of a previously issued Invoice, produced automatically when a refund or cancellation follows invoicing. An Invoice is never edited or deleted; it is only corrected by a Credit note.
_Avoid_: refund (the payment operation), negative invoice, cancelled invoice
