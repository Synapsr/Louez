...
- **Version applicative sur les events client** (`instrumentation-client.ts`) : quand `NEXT_PUBLIC_APP_VERSION` est renseignee par le pipeline de deploiement, PostHog recoit `$app_version` sur les events client pour relier les exceptions a une version. Au 2026-07-17, la propriete n'existe pas encore dans la taxonomie live ; le pipeline doit donc encore fournir un identifiant public stable (commit ou version de release).
